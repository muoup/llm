...
const float* get_addr(const ::matrix& mat, const size_t row, const size_t col);
...
void load_into(::matrix& matrix,
               const float* buffer,
               kernel_stream_t stream = nullptr);
void store_from(const ::matrix& matrix,
                float* buffer,
                kernel_stream_t stream = nullptr);
...
::matrix async_allocate(const size_t rows,
                        const size_t cols,
                        kernel_stream_t stream = nullptr);
...
float_device_ptr_t sum_of_squares(const ::matrix& mat,
                                  kernel_stream_t stream = nullptr);
float_device_ptr_t abssum(const ::matrix& mat,
                          kernel_stream_t stream = nullptr);
...
float_device_ptr_t absmax(const ::matrix& mat,
                          kernel_stream_t stream = nullptr);
float_device_ptr_t variance(const ::matrix& mat,
                            kernel_stream_t stream = nullptr);
...
::matrix dot_product(const ::matrix& a,
                     const ::matrix& b,
                     kernel_stream_t stream = nullptr);

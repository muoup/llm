...
void kernel::matrix::test_print() { ::test_print<<<1, 1>>>(); }
...
void kernel::matrix::set_all(::matrix &mat, float value) {
  ...
  kernel_set_all<<<blocks, threads_per_block>>>(mat.data, mat.stride, mat.rows,
                                                mat.cols, value);
}
...
void kernel::matrix::add_row_vector(::matrix &mat, const size_t row,
  ...
  add_row_vector_kernel<<<blocks, threads_per_block>>>(mat, row, vec, vec_row);
}

...
inline __device__ float *device_get_addr(float *data, const size_t stride,
                                         const size_t row, const size_t col) {
  return &(data[row + col * stride]);
}

inline __device__ float *device_get_addr(matrix_view data, const size_t row,
                                         const size_t col) {
...
inline __device__ void device_set(float *data, const size_t stride,
                                  const size_t row, const size_t col,
                                  const float value) {
  *(device_get_addr(data, stride, row, col)) = value;
}

inline __device__ void device_set(matrix_view data, const size_t row,
                                  const size_t col, const float value) {
  device_set(data.data, data.stride, row, col, value);
}

inline __device__ float device_get(const float *data, const size_t stride,
                                   const size_t row, const size_t col) {
  return *(device_get_addr(data, stride, row, col));
}

inline __device__ float device_get(const matrix_view data, const size_t row,
                                   const size_t col) {
...
inline __device__ void device_offset_elem(float *data, const size_t stride,
                                          const size_t row, const size_t col,
                                          float value) {
  *(device_get_addr(data, stride, row, col)) += value;
}

inline __device__ void device_offset_elem(matrix_view data, const size_t row,
                                          const size_t col, float value) {
  device_offset_elem(data.data, data.stride, row, col, value);
}

inline __device__ void
device_offset_elem_atomic(float *data, const size_t stride, const size_t row,
                          const size_t col, float value) {

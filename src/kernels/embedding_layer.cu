...
static __global__ void positional_encoding_kernel(
    float* input,
    size_t rows,
    size_t cols
) {
    ...
    if (token_i < rows && encoding_i < cols / 2) {
        const auto offset = encoding_i * 2;
        const auto inner
            = token_i
              / powf(10000.0f, offset / static_cast<float>(cols));
        input[token_i * cols + offset] = sinf(inner);
        input[token_i * cols + offset + 1] = cosf(inner);
    }
}
...
void kernel::embedding::positional_encoding(::matrix& input) {
    ...
    const dim3 gridSize(
        (input.rows + blockSize.x - 1) / blockSize.x,
        (input.cols / 2 + blockSize.y - 1) / blockSize.y
    );

    positional_encoding_kernel<<<gridSize, blockSize>>>(
        input.data,
        input.rows,
        input.cols
    );

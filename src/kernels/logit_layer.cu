...
kernel::logit_layer::LossResult kernel::logit_layer::compute_loss_gradient(
    ...
    cudaMemsetAsync(device_average_loss, 0, sizeof(float), from_kernel_stream(stream));
    ...
    compute_loss_gradient_kernel<<<grid_size, block_size, 0, from_kernel_stream(stream)>>>(
        d_actual, predictions, logit_loss_gradient, logit_bias_gradient,
        device_average_loss);

    float host_loss;
    cudaMemcpyAsync(&host_loss, device_average_loss, sizeof(float), cudaMemcpyDeviceToHost, from_kernel_stream(stream));

...
float norm_clip_factor(const matrix &gradient) {
    ...
    if (std::isinf(factor) || std::isnan(factor) || factor > 1.0f) {
        std::cout << "Warning: norm_clip_factor resulted in an invalid factor: "
                  << factor << '\n';
...
void regularize_weight_gradient(matrix &gradient, const matrix &weights) {
    ...
    for (size_t i = 0; i < gradient.rows; ++i) {
        for (size_t j = 0; j < gradient.cols; ++j) {
            ...
            const auto regularization_term
                = 2 * regularization_strength * weight_value;
...
matrix backpropogate_logit_row(llm &model, const matrix &last_ff_output,
    ...
    for (size_t i = 0; i < predictions.rows; ++i) {
        for (size_t j = 0; j < predictions.cols; ++j) {
            const auto delta_loss
                = predictions.get(i, j) - (j == actual[i + 1] ? 1.0f : 0.0f);
    ...
    matrix h_final_gradient
        = logit_loss_gradient.cross_multiplied(
            model.m_logit_layer.w.transposed());
    matrix logit_weight_gradient
        = last_ff_output.transposed().cross_multiplied(
            logit_loss_gradient);
...
matrix backpropogate_ff_layer(ff_layer &layer, const forward_result &result,
    ...
    const matrix a1_gradient = std::move(
        post_layer_gradient.cross_multiplied(layer.w2.transposed()));
    ...
    matrix w1_gradient
        = std::move(layer_input.transposed().cross_multiplied(z1_gradient));
    regularize_weight_gradient(w1_gradient, layer.w1);

    adjust_matrix(layer.w1, w1_gradient);

    auto input_gradient
        = std::move(z1_gradient.cross_multiplied(layer.w1.transposed()));
...
matrix backpropagate_attention_layer(attention_layer &layer,
    ...
    matrix wo_gradient
        = std::move(output.transposed().cross_multiplied(post_layer_gradient));
    matrix scores_gradient = std::move(
        post_layer_gradient.cross_multiplied(layer.wo.transposed()));
    ...
    matrix v_gradient
        = std::move(scores.transposed().cross_multiplied(scores_gradient));
    matrix scores_gradient_v
        = std::move(scores_gradient.cross_multiplied(v.transposed()));
...
#pragma omp parallel for
    ...
    matrix q_gradient = std::move(softmax_gradient.cross_multiplied(k));
    matrix k_gradient = std::move(softmax_gradient.transposed().cross_multiplied(q));
    ...
    matrix input_gradient
        = std::move(q_gradient.cross_multiplied(layer.wq.transposed()));
    input_gradient.add(
        k_gradient.cross_multiplied(layer.wk.transposed()));
    input_gradient.add(
        v_gradient.cross_multiplied(layer.wv.transposed()));
...
void backpropogate(llm &model, const training_data &data) {
    ...
    matrix previous_final_gradient = backpropogate_logit_row(
        model, data.logit_input, data.predictions, data.tokens);

    norm_clip(previous_final_gradient);

    for (int i = model.m_ff_layers.size() - 1; i >= 0; i--) {
        previous_final_gradient = backpropogate_ff_layer(
            model.m_ff_layers.at(i), data.forward_results.at(i),
            previous_final_gradient);
        norm_clip(previous_final_gradient);

        previous_final_gradient = backpropagate_attention_layer(
            model.m_attention_layers.at(i), data.attention_inputs.at(i),
            data.attention_forward_results.at(i), previous_final_gradient);
        norm_clip(previous_final_gradient);
    }
    ...
    std::cout << "Loss per token: "
              << (total_loss / static_cast<double>(data.tokens.size() - 1))
              << '\n';
}

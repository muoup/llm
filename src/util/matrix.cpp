...
using custom_matrix = blaze::CustomMatrix<float,
                                          blaze::AlignmentFlag::aligned,
...
#pragma omp parallel for
    for (size_t i = 0; i < this->rows; ++i) {
        ...
        for (size_t l = 0; l < this->cols; ++l) {
            s_dot += gradient.get(i, l) * this->get(i, l);
        }
        for (size_t j = 0; j < this->cols; ++j) {
            const float s_j = this->get(i, j);
            const float g_j = gradient.get(i, j);

...
int handle_init_model(int argc, char* argv[]) {
    ...
    if (output_path.empty() || tokenizer_path.empty()) {
        std::cerr << "Usage: ./llm init-model --output <path> --tokenizer <path> [--dimensions <n>] [--heads <n>] [--layers <n>]" << std::endl;
        return 1;
    }
    ...
    std::cout << "Loading tokenizer to determine vocab size: " << tokenizer_path << std::endl;
    ...
    // InferenceModel model = standard_attention_model(dimensions, _tokenizer.vocab_size(), layers, heads);

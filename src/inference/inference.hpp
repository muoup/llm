...
struct InferenceModel {
    ...
    std::vector<ForwardingResult> forwarding_results(
        std::span<const token_id_t> tokens,
        bool perf = false) const;

    token_id_t predict(std::span<const token_id_t> tokens,
                       float temperature = 1.0f,
                       bool perf = false) const;
    float train_on(std::span<const token_id_t> tokens,
                   std::span<const token_id_t> actual,
                   CentralOptimizer& optimizer,
                   bool perf = false);

    size_t parameter_count() const;

    InferenceModel(size_t dimensions,
                   size_t vocab_size,
                   DataType dtype = DataType::Float)
        : m_dimensions(dimensions),
          m_dtype(dtype),
          m_embedding_layer(dimensions, vocab_size, dtype),
          m_logit_layer(dimensions, vocab_size, dtype) {}
    ...
    size_t add_connection(size_t from_idx, size_t to_idx) {
        if (finalized) {
            throw std::runtime_error(
                "Cannot add connection to finalized model.");
        }
   ...
   private:
    InferenceModel()
        : finalized(false),
          ...
          m_embedding_layer(0, 0, DataType::Float),
          m_logit_layer(0, 0, DataType::Float) {}

...
RMSNorm::RMSNorm(std::unique_ptr<INode> inner_node,
    ...
    if (this->inner_node) {
        ...
        this->context_name = "RMSNorm";
    }
}

RMSNorm::RMSNorm() : streams(STREAMS_NEEDED) {}

size_t RMSNorm::parameterCount() const {
...
void RMSNorm::randomize(float min, float max) {
    gamma.set_all(1.0f);
...
NodeType RMSNorm::getType() const {
    return NodeType::RMSNorm;
}

ForwardingResult RMSNorm::forward(std::span<const matrix> inputs,
                                  bool perf) const {
    const matrix& input = inputs[0];

    auto start_norm = std::chrono::high_resolution_clock::now();
    auto results = kernel::layer_norm::rms_normalization(input, gamma, epsilon,
                                                         streams[0]);
    kernel::wait_for_streams(streams[0]);

    if (!inner_node) {
        auto fr = standardResult(
            matrix::construct_vec(results.normalized, results.inv_rms));
        return fr;
    }
    ...
    return_vec.emplace_back(std::move(results.inv_rms));
    return standardResult(std::move(return_vec));
}

std::vector<matrix> RMSNorm::backpropogate(const ForwardingResult& result,
                                           std::span<const matrix> inputs,
                                           std::span<const matrix> gradients,
                                           CentralOptimizer& optimizer,
                                           bool perf) {
    const matrix& layer_input = inputs[0];
    const matrix& inv_rms = result.outputs.rbegin()[0];

    std::vector<matrix> inner_backprop_outputs;

    auto start_inner = std::chrono::high_resolution_clock::now();
    const matrix& grad_normalized = [&]() -> const matrix& {
        const matrix& normalized_input = result.outputs.rbegin()[1];
    ...
    auto start_norm = std::chrono::high_resolution_clock::now();
    LOG_DEBUG("  RMSNorm Inputs: ");
    LOG_DEBUG("    layer_input norm: %f", layer_input.norm());
    LOG_DEBUG("    inv_rms norm: %f", inv_rms.norm());
    LOG_DEBUG("    grad_normalized norm: %f", grad_normalized.norm());

    kernel::layer_norm::RMSNormGradients results
        = kernel::layer_norm::rms_normalization_backward(
            layer_input, gamma, inv_rms, grad_normalized, epsilon, streams[0]);
...
void RMSNorm::save(std::ostream& out) const {
...
RMSNorm RMSNorm::load(std::istream& in) {
    RMSNorm layer;
